...
manifiestos:  any =[];
 public defaultItem: { text: string; value: number } = {
  ...
  cargarClientes(){
    ...
    this.comercialService.getClientesxGrupo(this.selectedGrupo.id).pipe(
      
      map((list) => list.map((x) => ({ label: x.razon_social, value: x.id })))

    ).subscribe({

      next: response=>  { this.listClientes = response }
    })
